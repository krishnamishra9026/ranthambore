...
import ChambalPricesService from '../../services/chambal-prices.service';
import objectService from '../../services/object.service';

import BaseController from '../base.controller';

class ChambalPricesController extends BaseController {
  constructor() {
    super('ChambalPrices');
  }
	 ...
	 */
	getChambalPricesData = (req, postData) => {
	  const event = {
	  	date: postData.date,
	    zone: postData.zone,
	 ...
	 */
	list = async req => {
	  const doc = await ChambalPricesService.list(req);  
	...
	create = async req => {
	  ...
	  const event = this.getChambalPricesData(req, postData);
	  event.active = postData.active;
	  event.createdBy = req.user._id;
	  const eventId = await ChambalPricesService.create(event);
	  if (!eventId) {
	 ...
	 */
	 ...
	 updateAvailability = async req => {
	 	const postData = req.body;
	 	const Event = await ChambalPricesService.getById(postData.id);
	 	 if (Event) {	   
	    Event.price = postData.value;
	    const doc = await ChambalPricesService.update(Event);
	    if (doc) {
	...
	update = async req => {
	  const postData = req.body;
	  const Event = await ChambalPricesService.getById(req.params.id);
	...
	getById = async req => {
	  ...
	  const category = await ChambalPricesService.model.findById(req.params.id);
	  if (category) {
	...
	parentLists = async req => {
	  ...
	  const cats = await ChambalPricesService.model.find(condition, selectList);
	...
	changeStatus = async req => {
	  ...
	  const update = await ChambalPricesService.changeStatus(data);
	  if (!update) {
...
export default ChambalPricesController;
