...
import ChambalPriceController from './chambal-prices.controller';
import { controllerHandler } from '../../../utils/async.handler';
import Validator from './chambal-prices.validation';

const router = express.Router();
const controller = new ChambalPriceController();
  ...
  static route() {
    ...
    router.post(
      ...
      [
        body('ids')
          .isLength({ min: 1 })
          .withMessage('please select chambal-prices to update status.'),
        body('ids.*')
          .isMongoId()
          .withMessage('Please provide valid unique identifier for chambal-prices.'),
        body('status')
          .isLength({ min: 1 })
