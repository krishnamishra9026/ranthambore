...
class EventsController extends BaseController {
  constructor() {
	 ...
	 */
	list = async req => {
	  const doc = await eventsService.list(req);  
	...
	create = async req => {
	  ...
	  const event = this.getEventsData(req, postData);
	  event.active = postData.active;
	  event.createdBy = req.user._id;
	  const eventId = await eventsService.create(event);
	  if (!eventId) {
	 ...
	 */
	 ...
	 updateAvailability = async req => {
	 	const postData = req.body;
	 	const Event = await eventsService.getById(postData.id);
	 	 if (Event) {	   
	    Event.availability = postData.value;
	    const doc = await eventsService.update(Event);
	    if (doc) {
	...
	getById = async req => {
	  ...
	  const category = await eventsService.model.findById(req.params.id);
	  if (category) {
	...
	parentLists = async req => {
	  ...
	  const cats = await eventsService.model.find(condition, selectList);
	...
	changeStatus = async req => {
	  ...
	  const update = await eventsService.changeStatus(data);
	  if (!update) {
