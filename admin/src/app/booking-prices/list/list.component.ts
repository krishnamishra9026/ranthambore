...
import { BookingPricesService } from '../booking-prices.service';
...
export class ListComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		private eventService: BookingPricesService,
	...
	private getBookingPrices() {
		this.eventService
			.getBookingPricesList({
			...
			.subscribe(
				(res: any) => {
					if (res.success && res.data) {
						this.selected = [];
						this.selectedItem = [];
						this.booking_services = res.data.docs;
				...
				(err) => {
					this.commonService.showError('', 'There is some problem on getting booking-services.');
				}
			);
	}
