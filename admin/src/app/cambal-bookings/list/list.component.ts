...
export class ListComponent implements OnInit, OnDestroy {
	...
	delete() {
		if (this.selectedItem && this.selectedItem.length) {
			const initialState = {
				title: 'Delete Brand',
				message: `Are you sure, you want to Delete <b>${this.selectedItem}</b> Booking?`
			};
				...
				if (res) {
					this.cambalBookingsService
						...
						.subscribe(
							(delRes: any) => {
								if (delRes.success) {
									this.commonService.showSuccess('', 'Booking deleted successfully.');
									this.getcustomers();
								}
							},
							(err) => {
								this.commonService.showError('', 'There is some problem on Deleting Booking.');
							}
						);
				}
			...
			this.toastrService.error('Please select atleast one Booking.');
		}
	}
